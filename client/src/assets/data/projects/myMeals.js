...
export const myMeals = {
  ...
  description: {
    [LOCALES.ENGLISH]:
      'Mobile application developed in React-Native, that allows you to check the available meal recipes from different meal categories, with the option to apply some filters, check the meal details (ingredients and steps) and include or exclude them from your favorite meals list. You can also check out your favorite meals. The app uses Redux for the state management, multiple types of navigation and a hard-coded mocked database.',
    [LOCALES.FRENCH]:
      "Application mobile, développée en React-Native, qui permet de consulter des recettes de cuisine parmi plusiuers catégories, avec la possibilité d'appliquer des filtres, de consulter les détails de la recette (ingrédients et étapes) et de l'inclure ou l'exclure de la liste des plats préférés. L'utilisateur peut également consulter cette liste de plats préférés. L'application utilise Redux pour la gestion de l'état, plusieurs types de navigation et une base de données JSON simulée",
    [LOCALES.SPANISH]:
      'Aplicación móvil, desarrollada en React-Native, que permite consultar varias recetas culinarias de distintas categorías, con la posibilidad de aplicar filtros, consultar los detalles de la receta (ingredientes y etapas) e incluso añadirle o quitarle de la lista de los platos favoritos. El usuario puede también consultar esta lista de rеcetas preferidas. La aplicación utiliza Redux para la gestión del estado, varios tipos de navegación, así que una base de datos JSON simulada.',
    [LOCALES.BULGARIAN]:
